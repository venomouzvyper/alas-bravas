      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.acompanamientos && item.acompanamientos.length > 0 && (
            <p className="text-brand-cream/25 text-[11px] mt-1.5 leading-tight">
...
export function CartaMenu({ items, mostrarPreciosBebidas }: CartaMenuProps) {
  ...
  return (
    <div style={{ background: '#160500', minHeight: '100vh' }}>
      ...
      {/* ── Footer CTA ─── */}
      <div className="px-4 py-12 text-center" style={{ background: '#0D0602', borderTop: '1px solid rgba(193,18,31,0.18)' }}>
        <div className="flex justify-center mb-4">
          <Ornamento color="rgba(255,183,3,0.28)" />
        </div>
        <p className="text-brand-cream/30 text-xs uppercase tracking-widest mb-5">¿Todo listo para pedir?</p>
        <a
          href="https://wa.me/50432462305"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2.5 px-8 py-4 rounded-full font-display text-xl tracking-widest text-white transition-transform active:scale-95"
        ...
        <p className="text-brand-cream/20 text-xs mt-5">La Cabaña, San Lorenzo · Lun – Dom 11 AM – 12 AM</p>
      </div>
    </div>
  );
}
